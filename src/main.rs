...
async fn main() -> Result<()> {
    ...
    if let Some(config) = scheduler_db_config { env::set_var("MITM_DB_CONFIG_JSON", config); }
    if let Some(key) = scheduler_master_key { env::set_var("MASTER_KEY", key); }
...
fn ipc_client(args: &JobArgs) -> Option<IpcClient> {
    Some(IpcClient { socket_path: env::var("SCHEDULER_SOCKET_PATH").ok()?, run_id: env::var("RUN_ID").ok()?.parse().ok()?, topic: args.topic.clone(), source_name: args.source_name.clone() })
}
